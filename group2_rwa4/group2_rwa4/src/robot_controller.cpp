...
robot_move_group_(robot_controller_options){
  ...
  // home_joint_pose_ = {1.18, 3.11, -1.60, 2.0, 4.30, -1.53, 0};
  ...
  gripper_subscriber_2 = gripper_nh_.subscribe(
...
void RobotController::qualityControlSensor1Callback(const osrf_gear::LogicalCameraImage::ConstPtr &image_msg) {
	...
	if(is_faulty_){
		ROS_WARN_STREAM("prodcut is faulty....and models.size is " << image_msg->models.size());
	}
...
void RobotController::SendRobot2Home() {
    // ros::Duration(2.0).sleep();
...
bool RobotController::DropPart(geometry_msgs::Pose part_pose) {
  ...
  // Going to kit here
  this->GoToTarget({temp_pose, part_pose},1);
  // ros::Duration(0.5).sleep();
...
// void RobotController::break_beam_callback_(const osrf_gear::Proximity::ConstPtr &msg) {
//     if (msg->object_detected) {
//         ROS_INFO("Break beam 2 triggered.");
// ROS_INFO_STREAM("BEAM"<<beam_);
